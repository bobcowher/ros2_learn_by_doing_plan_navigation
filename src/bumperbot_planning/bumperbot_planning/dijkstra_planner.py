...
from threading import active_count, excepthook
import rclpy
...
class DijkstraPlanner(Node):
    ...
    def goal_callback(self, pose: PoseStamped):
        ...
        self.visited_map_.data = [-1] * (self.map_.info.height 
                                         * self.map_.info.width)
        try:
            map_to_base_tf = self.tf_buffer.lookup_transform(self.map_.header.frame_id,
                                                             "base_footprint",
                                                             rclpy.time.Time())
    ...
    def plan(self, start, goal):
        ...
        while not pending_nodes.empty() and rclpy.ok():
            ...
            for dir_x, dir_y in explore_directions:
                ...
                if new_node not in visited_nodes and  
                    self.pose_on_map(new_node) and 
            ...
            self.visited_map_.data[self.pose_to_cell(active_node)] = 10 # 10 is a nice blue color in RVIZ
